...
void checkExceptions(char binaryStr[]) {
    ...
    if (combinedBits[0] == '0' && combinedBits[1] == '0') {
        ...
        SendMess_Up_Char(Mess_Adder_devicestatus, "0", 2, Mess_Length_devicestatus);
    } 
        ...
        SendMess_Up_Char(Mess_Adder_devicestatus, "1", 2, Mess_Length_devicestatus);
    } 
...
void Send_to_onenet(char* InBuf){
    ...
    while (count--)
   {
        ...
        printf("Uart2_Recver:\n%s\n",uartReadBuff);
        // if(strstr(uartReadBuff, "MIPLEVENT:0,26")) {
        //    break; 
        // }
        if(strstr(uartReadBuff, execute))
            Cloud_command(uartReadBuff);
...
{
    char send_NB_buffer[4096];                           //上传上云的消息
    ...
    while (count--)
    {
        ...
        usleep(500000);
        printf("__________________________\n");
        printf("NB_buffer: \n%s, length: %zu\n", NB_buffer, strlen(NB_buffer));

        // 使用 snprintf 防止缓冲区溢出
        int result = snprintf(send_NB_buffer, 8192,
                              "AT+MIPLNOTIFY=0,%s,3200,0,5750,1,%d,\"%s\",0,0,%d\n",
                              observe, strlen(NB_buffer), NB_buffer, i);

        if (result >= sizeof(send_NB_buffer)) {
            printf("Error: Buffer overflow detected in sprintf\n");
        } else if (result < 0) {
            printf("Error: sprintf failed\n");
        } else {
            printf("send_NB_buffer: \n");
            printf("%s", send_NB_buffer);
            printf(", length: %zu\n", strlen(send_NB_buffer));
            usleep(7000 * 1000);
            printf("\n");
            Send_to_onenet(send_NB_buffer);
        }
        memset(send_NB_buffer, 0, 4096); //清空缓冲区
        NB_buffer_index=0;
