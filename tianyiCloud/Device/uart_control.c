...
static void UartTask(void)
{
    ...
    while (count--)
    {
        ...
        // usleep(1500000);
        // printf("\n");
        // printf("over_________________\n");
        
        // 构建 AT+QLWDATASEND 指令格式
        sprintf(send_NB_buffer, "AT+QLWDATASEND=19,0,0,%d,%s,0x0100\r", strlen(NB_buffer), NB_buffer);
        usleep(9000000);
        
        IoTUartWrite(HI_UART_IDX_2, (unsigned char *)send_NB_buffer, strlen(send_NB_buffer));  //通过串口2发送数据
        printf("长度：%lu。已经发送\n", strlen(NB_buffer));
        //等待nb连上云平台
        usleep(1600000);
        ...
        Uart2_Read(uartReadBuff);
        usleep(1000000);
        memset(NB_buffer, 0, NB_BUFFER_SIZE); //清空缓冲区
...
void UartExampleEntry(void)
{
    ...
    attr.cb_size = 0U;
    attr.stack_mem = NULL;
    attr.stack_size = 16 * 1024; // 任务栈大小*1024 stack size 5*1024
